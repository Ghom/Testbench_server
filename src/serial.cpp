...
void start(void)
{
        ...
        snprintf(&deviceName[0], MAX_STR_LEN, "/dev/serial0");
        ...
        SetInterfaceAttribs(fd, B115200, 0, 20); /* set speed to 57600 bps, 8n1 (no parity), timeout 2 secs*/

        pthread_t readThread_t, sendThread_t;  /* thread variables */

        pthread_create(&readThread_t, NULL, readThread, (void *)&fd);
        pthread_create(&sendThread_t, NULL, sendThread, (void *)&fd);
